...
FILE="crailterasort100GB-4node-results-master4.csv"
touch $FILE

for disk_stat_name in ~/PAT/PAT-collecting-data/results/crailterasort100GB-4node/disk_stats/disk-avg-stats-*
do
   ...
   numeric_regex='^[0-9]+$'
       ...
       echo -n "'crailterasort100GB-4node', $num_cores, $ram, $disk1, $disk2, " >> $FILE
       echo -n ", , , , , , , , , , , ," >> $FILE
       if [ "$disk1" = "$disk2" ]; then
       ...
       seconds=`cat ~/PAT/PAT-collecting-data/results/$test_name/jobhistory/stdout | grep "Execution time:"  | awk '{ print $(3) }'| tr -dc '0-9'`
