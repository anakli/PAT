...
FILE="tpcds300-4node-results-master4.csv"
touch $FILE

for i in {1..13} 15 {17..22} {24..93} {96..99}
do
   #./post-proc-tpcds300-4node.sh q$i
   echo "processing tpds-q$i..."
   for disk_stat_name in ~/PAT/PAT-collecting-data/results/tpcds300-4node-q$i/disk_stats/disk-avg-stats-tpcds300*
   do
      test_name=${disk_stat_name#*"disk-avg-stats-"}
      test_name=${test_name%".csv"}
      query_name=${test_name#*"tpcds300-"}
